...
def main():
    ...
    if UPDATE_SPREADSHEET:
        harpSpreadsheet = HarpSpreadsheet(
    ...
    peripheral_template = TemplatePeripheralRepo(
        gh.get_repo(f"{ORGANIZATION}/peripheral.template")
    )
    ...
    who_am_i_df = pd.DataFrame(
        [
            ...
            for whoami in who_am_i_list.devices
        ],
    ...
    peripheral_repos = [
        PeripheralRepo(repo, peripheral_template)
        for repo in harp_organization.get_repos()
        if (
            ("peripheral." in repo.full_name)
            and "template" not in repo.full_name
            and not (repo.is_template)
